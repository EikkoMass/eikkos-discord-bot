...
import areContextMenusDifferent from '../../utils/areContextMenusDifferent.js';
  ...
  try {
    ...
    for(const localContext  of localContexts)
    {
      ...
      const existingContext = applicationCommands.cache.find(
        cmd => cmd.name === name && 
        (cmd.type === ApplicationCommandType.Message || cmd.type === ApplicationCommandType.User)
      );
      ...
      if(existingContext)
      {
        ...
        if (localContext.deleted)
        {
          console.log(`Skipping registering context menu "${name}" as it's set to delete`);
          continue;
        }
