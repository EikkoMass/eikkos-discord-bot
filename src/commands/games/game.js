...
async function search(client, interaction) {
  ...
  const fields = [
    {name: 'Keywords', value: (games[0].keywords.length > 10 ? games[0].keywords.splice(0, 10) : games[0].keywords).map(tag => tag.name).join(", ")},
    {name: 'Release Date', value: (new Date(games[0].first_release_date * 1000)).toDateString(), inline: true},
    {name: 'Genres', value: games[0].genres.map(tag => tag.name).join(", "), inline: true},
    {name: 'Companies', value: games[0].involved_companies.map(tag => tag.company.name).join(", ")},
  ];
  ...
  const embed = new EmbedBuilder()
    ...
    {
      embed
        .setThumbnail(`https://images.igdb.com/igdb/image/upload/t_cover_big_2x/${games[0].cover.image_id}.png`);
    }

    if(games[0].screenshots)
    {
      embed
        .setImage(`https://images.igdb.com/igdb/image/upload/t_thumb_2x/${games[0].screenshots[0].image_id}.jpg`);
    }
