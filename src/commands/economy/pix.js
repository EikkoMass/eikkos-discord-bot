...
async function request(client, interaction) {
  try {
    const words = await getLocalization(interaction.locale, `pix`);

    let userPixData = await Pix.findOne({
    ...
    const info =
      optDescription.length > 0
        ? "02" + getSafeLength(optDescription) + optDescription.substring(0, 50)
        : "";
    const account =
      "26" + (gui.length + pixKey.length + info.length) + gui + pixKey + info;
    ...
    for (i = 0; i < payload.length; i++) {
      ...
      if (c > 255) {
        ...
        return await replies.message.error(interaction, words.Error);
      }
    ...
    await replies.message.success(interaction, words.Creating);
    ...
    // fetch QR Code
    const size = 280;
    const qrImgRequest = await fetch(
      `https://api.qrserver.com/v1/create-qr-code/?size=${size}x${size}&data=${qrCodePix}`,
    );
    ...
    const infoFields = [
      {
        name: words.Value,
        value: formatMessage(words.CurrencyFormat, [optAmount]),
        inline: true,
      },
      { name: words.Date, value: new Date().toString() },
    ];
    ...
    const embed = new EmbedBuilder()
      .setTitle(formatMessage(words.Title, [optName]))
      .setColor("Random")
    ...
    interaction.channel.send({
      embeds: [embed],
      files: [{ attachment: Buffer.from(bufferImg), name: `pix.png` }],
    });
  } catch (e) {
    console.log(e);
    if (!interaction.replied) {
      await replies.message.error(
        interaction,
        formatMessage(words.ErrorN, [e]),
      );
    }
  }
}
...
async function register(client, interaction) {
  try {
    ...
    let pixRegister = await Pix.findOne({
      guildId: interaction.guild.id,
      userId: interaction.user.id,
    });
    ...
    if (pixRegister) {
      ...
      return await replies.message.success(interaction, words.PixInfoEdited);
    }
    ...
    await replies.message.success(interaction, words.PixInfoCreated);
  } catch (e) {
    console.log(e);
  }
}
